#ifndef IONCH_EVALUATOR_INDUCED_CHARGE_HPP
#define IONCH_EVALUATOR_INDUCED_CHARGE_HPP
...
// Induced Charge Computation evaluator.
//
...
//   the simulation.
class induced_charge : public base_evaluator
 {
   private:
    // The ICC A matrix.
    icc_matrix amx_;
    ...
    //  The set of patches on the surface
    std::unique_ptr< icc_surface_grid > grid_;
   ...
   private:
    //no copy
    induced_charge(const induced_charge & source) = delete;

    //no move
    induced_charge(induced_charge && source) = delete;


   public:
    virtual ~induced_charge()
    {}


   private:
    //no assignment
    induced_charge & operator=(const induced_charge & source) = delete;
  ...
  friend class boost::serialization::access;
   ...
   public:
    ...
    {
       return this->amx_.size();
    }

    virtual std::string type_label() const override
    {
      return induced_charge::type_label_();
    }
    ...
    void create_amx(std::unique_ptr< icc_surface_grid >& grid);
    ...
    //  see ionch::iccgrid::add_XXXX for patch::goXXXX
    //  see ionch::XXXX_integrator types for patch::intXXXX
    void create_amx_f(double zl1, double rl1, double rlvest);
